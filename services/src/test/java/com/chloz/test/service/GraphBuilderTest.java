...
class GraphBuilderTest {
    ...
    void createEntityGraphWithAllSubGraphSimpleFields() {
        ...
        assertTrue(List.of("id", "login", "password", "createdDate", "disabled", "lastModifiedDate", "deleted", "picture", "createdBy", "lastModifiedBy").stream()
                .allMatch(s -> graph.getAttributeNodes().stream().anyMatch(at -> at.getAttributeName().equals(s))));
        // SubGraph exists for all child properties : (picture, createdBy, lastModifiedBy)
        List<AttributeNode<?>> subGraphs = this.getSubEntityGraph(graph.getAttributeNodes());
        assertEquals(3, subGraphs.size());
        assertTrue(subGraphs.stream().allMatch(an -> List.of("picture", "createdBy", "lastModifiedBy").contains(an.getAttributeName())));
    ...
    private List<AttributeNode<?>> getSubEntityGraph(List<AttributeNode<?>> attributeNodes) {
        return attributeNodes.stream().filter(attributeNode -> attributeNode.getSubgraphs() != null && !attributeNode.getSubgraphs().isEmpty()).toList();
    }
}